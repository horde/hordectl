...
class Show implements Module, ModuleUsage
{
    ...
    {
        $this->cli->writeln();
        $this->cli->message('Display admin_secret', 'cli.success');
        $this->cli->writeln();
        ...
        if (!$currentTargetName) {
            $this->cli->message('ERROR: No target configured', 'cli.error');
            $this->cli->writeln();
        ...
        if (!$target) {
            $this->cli->message(sprintf('ERROR: Target "%s" not found', $currentTargetName), 'cli.error');
            $this->cli->writeln();
            return;
        }
        ...
        if (!$target->adminSecret) {
            $this->cli->message('No admin_secret configured for this target', 'cli.warning');
            $this->cli->writeln();
        ...
        $this->cli->message('Current admin_secret:', 'cli.success');
        $this->cli->writeln();
        $this->cli->writeln('  ' . $target->adminSecret);
        $this->cli->writeln();
        $this->cli->message('⚠ WARNING: Keep this secret secure!', 'cli.warning');
        $this->cli->writeln();
