...
class Logger implements Module, ModuleUsage
{
    ...
    public function handle(array $argv = [])
    {
        ...
        if (!$target->supportsApiCommands()) {
            ...
            $this->cli->writeln('The logger test requires a configured Horde web endpoint.');
        ...
        $this->cli->writeln(str_repeat('=', strlen('Logger Test')));
        $this->cli->writeln();

        try {
            $result = $this->apiClient->checkHealth('logger');
            $this->displayHealthCheck($result);
            return true;
        } catch (Exception $e) {
            $this->displayApiError($e);
            throw new TestModuleException("Logger test failed: {$e->getMessage()}", 0, $e);
        }
    }
}
